...
namespace StorybrewScripts
{
    public class Particles : StoryboardObjectGenerator
    {
        ...
        [Configurable] public float ScaleRandom = 0f;
        ...
        public override void Generate()
        {
            ...
            for (var i = 0; i < ParticleCount; i++)
            {
                ...
                particle.Rotate(startTime, Random(-Math.PI, Math.PI));
                if (color.R != 1 || color.G != 1 || color.B != 1)
                    particle.Color(startTime, color);

                particle.Scale(startTime, Scale.X + Random(-ScaleRandom, ScaleRandom));
